...
- `lib/messaging/email` for provider selection and sending
- `components/emails` for React Email templates
- `components/emails/subjects.ts` for email subject lines
...
`lib/messaging/email/validation.ts` provides:
